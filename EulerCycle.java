...
import java.util.ArrayList;
    ...
    public EulerCycle(ArrayList<Integer>[] graph) {
         ...
         */
        public boolean checkIfEulerCycle() {
            boolean ans = true;
        ...
        while (i < graph.length) {
            if (graph[i].size() % 2 != 0) {
                ...
                while (flag && i < graph.length) {
                    if (graph[i].size() % 2 != 0) {
                        Edges.add(new Edge(begin, i)); // new Edge between the 2 vertexes with number odd Edges
                        flag = false;
                        graph[begin].add(i); // add this Edge to nei list of begin vertex
                        degrees[begin]++; // update the degree of begin vertex
                        graph[i].add(begin); // add this Edge to nei list of i(end) vertex
                        degrees[i]++; // update the degree of i(end) vertex
                    } else i++;
                }
            }
            i++;
