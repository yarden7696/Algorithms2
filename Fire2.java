    ...
    public class Fire2 {
        ...
        public static void fire2(MyVector[] tree){
            ...
            for (int i = 0; i<len; i++){ // init thw queue with the all leaves
                degrees[i] = tree[i].size(); // tree[i].size() is like in tirgul 9 page 3- example v2 its size 3 so in
                                            // degrees[2] will be 3
            ...
            while (!leavesQ.isEmpty()){ //O(n)- while the queue still full
                ...
                for (int j=0; j<tree[leaf].size(); j++){ // in this for loop we running of all the neigh of this leaf
                    ...
                    if(degrees[neigh] == 1) { // the neigh is leave
                        leavesQ.add(neigh); // add the neigh to the queue
                        levels[neigh] = levels[leaf] + 1;
                        maxLevel = Math.max(maxLevel, levels[neigh]);
                    }
                }
            }
